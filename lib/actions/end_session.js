  ...
  async function endSessionChecks(ctx, next) {
    ...
    if (params.id_token_hint) {
      try {
        ...
        throw new InvalidRequest('could not decode id_token_hint', undefined, err.message, '036');
      }
      const { payload: { aud: clientId } } = ctx.oidc.entities.IdTokenHint;

      if (params.client_id && params.client_id !== clientId) {
        throw new InvalidRequest('client_id does not match the provided id_token_hint', undefined, undefined, '037');
      }
      client = await ctx.oidc.provider.Client.find(clientId);
      if (!client) {
        throw new InvalidClient('unrecognized id_token_hint audience', 'client not found', '002');
      }
      try {
        ...
        throw new InvalidRequest('could not validate id_token_hint', undefined, err.message, '038');
      }
      ctx.oidc.entity('Client', client);
    } else if (params.client_id) {
      client = await ctx.oidc.provider.Client.find(params.client_id);
      if (!client) {
        throw new InvalidClient('client is invalid', 'client not found', '003');
      }
      ctx.oidc.entity('Client', client);
    }
    ...
    if (client && params.post_logout_redirect_uri !== undefined) {
      if (!client.postLogoutRedirectUriAllowed(params.post_logout_redirect_uri)) {
        throw new InvalidRequest('post_logout_redirect_uri not registered', undefined, undefined, '039');
      }
...
export const confirm = [
  ...
  async function checkLogoutToken(ctx, next) {
    if (!ctx.oidc.session.state) {
      throw new InvalidRequest('could not find logout details', undefined, undefined, '040');
    }
    if (ctx.oidc.session.state.secret !== ctx.oidc.params.xsrf) {
      throw new InvalidRequest('xsrf token invalid', undefined, undefined, '041');
    }
    await next();
  },
