...
const validateRegistrationAccessToken = [
  ...
  async function validateRegistrationAccessToken(ctx, next) {
    ...
    ctx.assert(regAccessToken, new InvalidToken('token not found', '001'));
    ...
    if (!client || client.clientId !== regAccessToken.clientId) {
      ...
      throw new InvalidToken('client mismatch', '002');
    }
  ...
  async function validateInitialAccessToken(ctx, next) {
    ...
    switch (initialAccessToken && typeof initialAccessToken) {
      case 'boolean': {
        ...
        ctx.assert(iat, new InvalidToken('initial access token not found', '003'));
      ...
      case 'string': {
        ...
        ctx.assert(valid, new InvalidToken('invalid initial access token value', '004'));
        break;
      }
      default:
    }
...
export const get = [
  ...
  async function clientReadResponse(ctx, next) {
    if (ctx.oidc.client.noManage) {
      throw new InvalidRequest('client does not have permission to read its record', 403, undefined, '043');
    }
...
export const put = [
  ...
  async function forbiddenFields(ctx, next) {
    ...
    ctx.assert(!hit, new InvalidRequest(template`request MUST NOT include the ${hit} field`, undefined, undefined, '044'));
    await next();
  },

  async function equalChecks(ctx, next) {
    ctx.assert(ctx.oidc.body.client_id === ctx.oidc.client.clientId, new InvalidRequest('provided client_id does not match the authenticated client\'s one', undefined, undefined, '045'));
    ...
    if ('client_secret' in ctx.oidc.body) {
      ...
      ctx.assert(clientSecretValid, new InvalidRequest('provided client_secret does not match the authenticated client\'s one', undefined, undefined, '046'));
    }
  ...
  async function clientUpdateResponse(ctx, next) {
    if (ctx.oidc.client.noManage) {
      throw new InvalidRequest('client does not have permission to update its record', 403, undefined, '047');
    }
...
export const del = [
  ...
  async function clientRemoveResponse(ctx, next) {
    if (ctx.oidc.client.noManage) {
      throw new InvalidRequest('client does not have permission to delete its record', 403, undefined, '048');
    }
