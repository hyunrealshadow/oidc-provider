...
export default [
  ...
  async function validateAccessToken(ctx, next) {
    ...
    ctx.assert(accessToken, new InvalidToken('access token not found', '005'));

    ctx.oidc.entity('AccessToken', accessToken);

    const { scopes } = accessToken;
    if (!scopes.size || !scopes.has('openid')) {
      throw new InsufficientScope('access token missing openid scope', 'openid', undefined, '001');
    }
    ...
    if (accessToken['x5t#S256']) {
      ...
      if (!cert || accessToken['x5t#S256'] !== certificateThumbprint(cert)) {
        throw new InvalidToken('failed x5t#S256 verification', '006');
      }
    }
    ...
    if (dPoP) {
      ...
      ctx.assert(unique, new InvalidToken('DPoP proof JWT Replay detected', '007'));
    }

    if (accessToken.jkt && (!dPoP || accessToken.jkt !== dPoP.thumbprint)) {
      throw new InvalidToken('failed jkt verification', '008');
    }
  ...
  function validateAudience(ctx, next) {
    ...
    if (accessToken.aud !== undefined) {
      throw new InvalidToken('token audience prevents accessing the userinfo endpoint', '009');
    }
  ...
  async function validateScope(ctx, next) {
    if (ctx.oidc.params.scope) {
      ...
      if (missing.length !== 0) {
        throw new InsufficientScope('access token missing requested scope', missing.join(' '), undefined, '002');
      }
    }
    await next();
  },
  ...
  async function loadClient(ctx, next) {
    ...
    ctx.assert(client, new InvalidToken('associated client not found', '010'));
  ...
  async function loadAccount(ctx, next) {
    ...
    ctx.assert(account, new InvalidToken('associated account not found', '011'));
  ...
  async function loadGrant(ctx, next) {
    ...
    if (!grant) {
      throw new InvalidToken('grant not found', '012');
    }

    if (grant.isExpired) {
      throw new InvalidToken('grant is expired', '013');
    }

    if (grant.clientId !== ctx.oidc.accessToken.clientId) {
      throw new InvalidToken('clientId mismatch', '014');
    }

    if (grant.accountId !== ctx.oidc.accessToken.accountId) {
      throw new InvalidToken('accountId mismatch', '015');
    }
