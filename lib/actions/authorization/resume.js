...
export default async function resumeAction(allowList, resumeRouteName, ctx, next) {
  ...
  if (!cookieId) {
    throw new errors.SessionNotFound('authorization request has expired', undefined, undefined, '001');
  }

  const interactionSession = await ctx.oidc.provider.Interaction.find(cookieId);
  if (!interactionSession) {
    throw new errors.SessionNotFound('interaction session not found', undefined, undefined, '002');
  }
  ctx.oidc.entity('Interaction', interactionSession);

  if (cookieId !== interactionSession.uid) {
    throw new errors.SessionNotFound('authorization session and cookie identifier mismatch', undefined, undefined, '003');
  }
  ...
  if (originSession?.uid && originSession.uid !== session.uid) {
    throw new errors.SessionNotFound('interaction session and authentication session mismatch', undefined, undefined, '004');
  }
