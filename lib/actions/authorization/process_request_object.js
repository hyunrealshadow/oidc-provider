...
export default async function processRequestObject(PARAM_LIST, rejectDupesMiddleware, ctx, next) {
  ...
  if (client.requirePushedAuthorizationRequests && route !== 'pushed_authorization_request' && !pushedRequestObject) {
    throw new InvalidRequest('Pushed Authorization Request must be used', undefined, undefined, '029');
  }
  ...
  if (
    ...
    throw new InvalidRequest('Request Object must be used by this client', undefined, undefined, '030');
  }
  ...
  if (features.encryption.enabled && params.request.split('.').length === 5) {
    if (isBackchannelAuthentication) {
      throw new InvalidRequest('Encrypted Request Objects are not supported by CIBA', undefined, undefined, '031');
    }
    ...
    try {
      ...
      throw new InvalidRequestObject('could not decrypt request object', err.message, '002');
    }
  }
  ...
  try {
    ...
    throw new InvalidRequestObject('could not parse Request Object', err.message, '003');
  }
  ...
  if (request.request !== undefined || request.request_uri !== undefined) {
    throw new InvalidRequestObject('Request Object must not contain request or request_uri properties', undefined, '004');
  }
  ...
  if (
    ...
    throw new InvalidRequestObject('request response_type must equal the one in request parameters', undefined, '005');
  }
  ...
  if (
    ...
    throw new InvalidRequestObject('request client_id must equal the one in request parameters', undefined, '006');
  }

  if (route === 'pushed_authorization_request') {
    if (request.client_id !== ctx.oidc.client.clientId) {
      throw new InvalidRequestObject('request client_id must equal the authenticated client\'s client_id', undefined, '007');
    }
  }

  if (request.client_id !== undefined && request.client_id !== client.clientId) {
    throw new InvalidRequestObject('request client_id mismatch', undefined, '008');
  }

  if (!pushedRequestObject && !conf.requestObjectSigningAlgValues.includes(alg)) {
    throw new InvalidRequestObject('unsupported signed request alg', undefined, '009');
  }
  ...
  if (!pushedRequestObject && client[prop] && alg !== client[prop]) {
    throw new InvalidRequestObject('the preregistered alg must be used in request or request_uri', undefined, '010');
  }
  ...
  if (fapiProfile) {
    if (!('exp' in payload)) {
      throw new InvalidRequestObject("Request Object is missing the 'exp' claim", undefined, '011');
    }

    if (fapiProfile === '1.0 Final') {
      if (!('aud' in payload)) {
        throw new InvalidRequestObject("Request Object is missing the 'aud' claim", undefined, '012');
      }
      if (!('nbf' in payload)) {
        throw new InvalidRequestObject("Request Object is missing the 'nbf' claim", undefined, '013');
      }
      const diff = payload.exp - payload.nbf;
      if (Math.sign(diff) !== 1 || diff > 3600) {
        throw new InvalidRequestObject("Request Object 'exp' claim too far from 'nbf' claim", undefined, '014');
      }
    }
  }
  ...
  if (isBackchannelAuthentication) {
    for (const claim of ['exp', 'iat', 'nbf', 'jti']) {
      if (!(claim in payload)) {
        throw new InvalidRequestObject(template`Request Object is missing the '${claim}' claim`, undefined, '015');
      }
    }
    ...
    if (fapiProfile) {
      const diff = payload.exp - payload.nbf;
      if (Math.sign(diff) !== 1 || diff > 3600) {
        throw new InvalidRequestObject("Request Object 'exp' claim too far from 'nbf' claim", undefined, '016');
      }
    }
  }
  ...
  try {
    ...
    throw new InvalidRequestObject('Request Object claims are invalid', err.message, '017');
  }
  ...
  if (pushedRequestObject) {
    ...
    try {
      ...
      throw new InvalidRequestObject('could not validate Request Object', err.message, '018');
    }
  }
  ...
  if (!pushedRequestObject && payload.jti && payload.exp && payload.iss) {
    ...
    if (!unique) {
      throw new InvalidRequestObject('request object replay detected', undefined, '019');
    }
  }
  ...
  if (trusted) {
    ...
    throw new InvalidRequestObject('Request Object from insecure request_uri must be signed and/or symmetrically encrypted', undefined, '020');
  }
