...
export default function checkOpenIdScope(PARAM_LIST, ctx, next) {
  ...
  if (PARAM_LIST.has('response_type') && ctx.oidc.params.response_type.includes('id_token')) {
    throw new InvalidRequest('openid scope must be requested for this response_type', undefined, undefined, '013');
  }

  GATED_CLIENT.forEach(([prop, msg]) => {
    if (ctx.oidc.client[prop]) {
      throw new InvalidRequest(template`openid scope must be requested for clients with ${msg}`, undefined, undefined, '014');
    }
  });

  GATED.forEach((param) => {
    if (ctx.oidc.params[param] !== undefined) {
      throw new InvalidRequest(template`openid scope must be requested when using the ${param} parameter`, undefined, undefined, '015');
    }
  });

  if (ctx.oidc.route === 'backchannel_authentication') {
    throw new InvalidRequest('openid scope must be requested for this request', undefined, undefined, '016');
  }
