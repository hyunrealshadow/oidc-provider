...
export default function checkResponseType(ctx, next) {
  ...
  if (!supported.includes(params.response_type)) {
    throw new UnsupportedResponseType(undefined, undefined, '001');
  }

  if (!ctx.oidc.client.responseTypeAllowed(params.response_type)) {
    throw new UnauthorizedClient('requested response_type is not allowed for this client', undefined, '003');
  }
