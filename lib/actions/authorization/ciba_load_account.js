...
export default async function cibaLoadAccount(ctx, next) {
  ...
  if (!length) {
    throw new InvalidRequest('missing one of required parameters login_hint_token, id_token_hint, or login_hint', undefined, undefined, '027');
  } else if (length !== 1) {
    throw new InvalidRequest('only one of required parameters login_hint_token, id_token_hint, or login_hint must be provided', undefined, undefined, '028');
  }
  ...
  if (!accountId) {
    throw new UnknownUserId('could not identify end-user', undefined, '001');
  }
