...
export default async function fetchRequestUri(ctx, next) {
  ...
  if (params.request_uri !== undefined) {
    ...
    try {
      ...
      throw new InvalidRequestUri('invalid request_uri scheme', undefined, '001');
    }
    ...
    if (
      ...
      if (!ctx.oidc.client.requestUriAllowed(params.request_uri)) {
        throw new InvalidRequestUri('provided request_uri is not allowed', undefined, '002');
      }
    }
    ...
    try {
      ...
      throw new InvalidRequestUri('could not load or parse request_uri', err.message, '003');
    }
  }
