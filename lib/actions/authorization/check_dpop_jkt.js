...
export default async function checkDpopJkt(ctx, next) {
  ...
  if (dPoP) {
    ...
    ctx.assert(unique, new InvalidRequest('DPoP proof JWT Replay detected', undefined, undefined, '009'));

    if (params.dpop_jkt && params.dpop_jkt !== dPoP.thumbprint) {
      throw new InvalidRequest('DPoP proof key thumbprint does not match dpop_jkt', undefined, undefined, '010');
