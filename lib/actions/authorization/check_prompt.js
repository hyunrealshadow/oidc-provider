  ...
  if (ctx.oidc.params.prompt !== undefined) {
    ...
    for (const prompt of prompts) {
      if (!supported.has(prompt)) {
        throw new InvalidRequest('unsupported prompt value requested', undefined, undefined, '021');
      }
    }

    if (prompts.has('none') && prompts.size !== 1) {
      throw new InvalidRequest('prompt none must only be used alone', undefined, undefined, '022');
    }
  }
