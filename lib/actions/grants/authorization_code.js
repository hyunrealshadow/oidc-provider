...
export const handler = async function authorizationCodeHandler(ctx, next) {
  ...
  if (!code) {
    throw new InvalidGrant('authorization code not found', '001');
  }
  ...
  if (code.isExpired) {
    throw new InvalidGrant('authorization code is expired', '003');
  }
  ...
  if (!grant) {
    throw new InvalidGrant('grant not found', '004');
  }

  if (grant.isExpired) {
    throw new InvalidGrant('grant is expired', '005');
  }
  ...
  if (grant.clientId !== ctx.oidc.client.clientId) {
    throw new InvalidGrant('client mismatch', '008');
  }

  if (code.redirectUri !== ctx.oidc.params.redirect_uri) {
    throw new InvalidGrant('authorization code redirect_uri mismatch', '009');
  }

  if (code.consumed) {
    await revoke(ctx, code.grantId);
    throw new InvalidGrant('authorization code already consumed', '010');
  }
  ...
  if (!account) {
    throw new InvalidGrant('authorization code invalid (referenced account not found)', '011');
  }
  ...
  if (code.dpopJkt && !dPoP) {
    throw new InvalidGrant('missing DPoP proof JWT', '013');
  }
  ...
  if (dPoP) {
    ...
    ctx.assert(unique, new InvalidGrant('DPoP proof JWT Replay detected', '014'));

    if (code.dpopJkt && code.dpopJkt !== dPoP.thumbprint) {
      throw new InvalidGrant('DPoP proof key thumbprint does not match dpop_jkt', '015');
    }
