...
export const handler = async function cibaHandler(ctx, next) {
  ...
  if (!request) {
    throw new InvalidGrant('backchannel authentication request not found', '016');
  }
  ...
  if (request.isExpired) {
    throw new ExpiredToken('backchannel authentication request is expired', undefined, '001');
  }
  ...
  if (request.consumed) {
    ...
    throw new InvalidGrant('backchannel authentication request already consumed', '017');
  }
  ...
  if (!account) {
    throw new InvalidGrant('backchannel authentication request invalid (referenced account not found)', '018');
  }
