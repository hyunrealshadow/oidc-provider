...
export const handler = async function refreshTokenHandler(ctx, next) {
  ...
  if (!grant) {
    throw new InvalidGrant('grant not found', '003');
  }

  if (grant.isExpired) {
    throw new InvalidGrant('grant is expired', '004');
  }
  ...
  if (ctx.oidc.params.scope) {
    ...
    if (missing.length !== 0) {
      throw new InvalidScope(template`refresh token missing requested ${[['scope', 'scopes', missing.length], { type: 'pluralize' }]}`, missing.join(' '), undefined, '002');
    }
  }
