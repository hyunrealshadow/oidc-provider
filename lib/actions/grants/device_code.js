...
export const handler = async function deviceCodeHandler(ctx, next) {
  ...
  if (!code) {
    throw new InvalidGrant('device code not found', '019');
  }
  ...
  if (code.isExpired) {
    throw new ExpiredToken('device code is expired', undefined, '002');
  }
  ...
  if (code.consumed) {
    ...
    throw new InvalidGrant('device code already consumed', '020');
  }
  ...
  if (!account) {
    throw new InvalidGrant('device code invalid (referenced account not found)', '021');
  }
