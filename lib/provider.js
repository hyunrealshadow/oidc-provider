...
import { SessionNotFound, OIDCProviderError, setErrorPrefix } from './helpers/errors.js';
...
async function getInteraction(req, res) {
  ...
  if (!id) {
    throw new SessionNotFound('interaction session id cookie not found', undefined, undefined, '005');
  }
  const interaction = await this.Interaction.find(id);
  if (!interaction) {
    throw new SessionNotFound('interaction session not found', undefined, undefined, '006');
  }

  if (interaction.session?.uid) {
    const session = await this.Session.findByUid(interaction.session.uid);
    if (!session) {
      throw new SessionNotFound('session not found', undefined, undefined, '007');
    }
    if (interaction.session.accountId !== session.accountId) {
      throw new SessionNotFound('session principal changed', undefined, undefined, '008');
    }
  }
...
class Provider extends events.EventEmitter {
  ...
  constructor(issuer, setup) {
    ...
    if (configuration.errorPrefix) {
      setErrorPrefix(configuration.errorPrefix);
    }
