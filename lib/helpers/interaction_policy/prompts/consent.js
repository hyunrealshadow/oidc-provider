...
export default () => new Prompt(
  ...
  new Check('native_client_prompt', 'native clients require End-User interaction', '001', 'interaction_required', (ctx) => {
    const { oidc } = ctx;
    if (
  ...
  new Check('op_scopes_missing', 'requested scopes not granted', '002', (ctx) => {
    const { oidc } = ctx;
  ...
  new Check('op_claims_missing', 'requested claims not granted', '003', (ctx) => {
    const { oidc } = ctx;
  ...
  new Check('rs_scopes_missing', 'requested scopes not granted', '004', (ctx) => {
    const { oidc } = ctx;
