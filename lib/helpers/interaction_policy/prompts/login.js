...
export default () => new Prompt(
  ...
  new Check('no_session', 'End-User authentication is required', '001', (ctx) => {
    const { oidc } = ctx;
  ...
  new Check('max_age', 'End-User authentication could not be obtained', '002', (ctx) => {
    const { oidc } = ctx;
  ...
  new Check(
    ...
    (ctx) => {
      ...
      if (!Array.isArray(oidc.claims.id_token.acr.values)) {
        throw new errors.InvalidRequest('invalid claims.id_token.acr.values type', undefined, undefined, '063');
      }
