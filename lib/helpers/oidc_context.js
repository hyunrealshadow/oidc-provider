...
export default function getContext(provider) {
  ...
  class OIDCContext extends events.EventEmitter {
    ...
    getAccessToken({
      ...
      if (!length) {
        throw new InvalidRequest('no access token provided', undefined, undefined, '051');
      }

      if (length > 1) {
        throw new InvalidRequest('access token must only be provided using one mechanism', undefined, undefined, '052');
      }

      if (!acceptQueryParam && mechanism === 'query') {
        throw new InvalidRequest('access tokens must not be provided via query parameter', undefined, undefined, '053');
      }
      ...
      if (mechanism === 'header') {
        ...
        if (dpop && scheme.toLowerCase() !== 'dpop') {
          throw new InvalidRequest('authorization header scheme must be `DPoP` when DPoP is used', undefined, undefined, '055');
        } else if (!dpop && scheme.toLowerCase() === 'dpop') {
          throw new InvalidRequest('`DPoP` header not provided', undefined, undefined, '056');
        } else if (!dpop && scheme.toLowerCase() !== 'bearer') {
          throw new InvalidRequest('authorization header scheme must be `Bearer`', undefined, undefined, '057');
        }
      ...
      if (dpop && mechanism !== 'header') {
        throw new InvalidRequest('`DPoP` tokens must be provided via an authorization header', undefined, undefined, '058');
      }
