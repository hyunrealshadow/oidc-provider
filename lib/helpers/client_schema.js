      ...
      if (metadata.subject_type === 'pairwise') {
        for (const grant of ['urn:ietf:params:oauth:grant-type:device_code', 'urn:openid:params:grant-type:ciba']) {
          if (metadata.grant_types.includes(grant) && !['private_key_jwt', 'self_signed_tls_client_auth'].includes(metadata.token_endpoint_auth_method)) {
            metadata.invalidate(template`pairwise ${grant} clients must utilize private_key_jwt or self_signed_tls_client_auth token endpoint authentication methods`, undefined, '101');
          }
        }
      }
  ...
  class Schema {
    constructor(
      ...
      ['default_max_age', 'client_secret_expires_at'].forEach((prop) => {
        if (this[prop] !== undefined) {
          if (!Number.isSafeInteger(this[prop]) || Math.sign(this[prop]) === -1) {
            this.invalidate(template`${prop} must be a non-negative integer`, undefined, '102');
          }
        }
      });
      ...
      if (this.grant_types.some((type) => ['authorization_code', 'implicit'].includes(type)) && !this.response_types.length) {
        this.invalidate('response_types must contain members', undefined, '103');
      }

      if (responseTypes.length && !this.redirect_uris.length) {
        this.invalidate('redirect_uris must contain members', undefined, '104');
      }

      if (responseTypes.includes('code') && !this.grant_types.includes('authorization_code')) {
        this.invalidate("grant_types must contain 'authorization_code' when code is amongst response_types", undefined, '105');
      }

      if (responseTypes.includes('token') || responseTypes.includes('id_token')) {
        if (!this.grant_types.includes('implicit')) {
          this.invalidate("grant_types must contain 'implicit' when 'id_token' or 'token' are amongst response_types", undefined, '106');
        }
      }

      {
        ...
        if (length > 1) {
          this.invalidate('only one proof of possession mechanism can be made required at a time', undefined, '107');
        }
      }

      {
        ...
        if (this.token_endpoint_auth_method === 'tls_client_auth') {
          if (length === 0) {
            this.invalidate('tls_client_auth requires one of the certificate subject value parameters', undefined, '108');
          }

          if (length !== 1) {
            this.invalidate('only one tls_client_auth certificate subject value must be provided', undefined, '109');
          }
        } else {
      ...
      if (this.jwks !== undefined && this.jwks_uri !== undefined) {
        this.invalidate('jwks and jwks_uri must not be used at the same time', undefined, '110');
      }
    ...
    invalidate(message, code, errno) { // eslint-disable-line class-methods-use-this, no-unused-vars
      throw new InvalidClientMetadata(message, undefined, errno);
    }
    ...
    required() {
      ...
      checked.forEach((prop) => {
        if (!this[prop]) {
          this.invalidate(template`${prop} is mandatory property`, undefined, '111');
        }
      });
      ...
      if (requireJwks && !this.jwks && !this.jwks_uri) {
        this.invalidate('jwks or jwks_uri is mandatory for this client', undefined, '112');
      }
    }
        ...
        if (this[prop] !== undefined) {
          ...
          (isAry ? this[prop] : [this[prop]]).forEach((val) => {
            if (typeof val !== 'string' || !val.length) {
              if (isAry) {
                this.invalidate(template`${prop} must only contain strings`, undefined, '113');
              } else {
                this.invalidate(template`${prop} must be a non-empty string if provided`, undefined, '114');
              }
            }
          });
        }
        ...
        if (this[prop] !== undefined) {
          ...
          (isAry ? this[prop] : [this[prop]]).forEach((val) => {
            ...
            if (!validUrl[method](val)) {
              if (isAry) {
                this.invalidate(template`${prop} must only contain ${type} uris`, undefined, '115');
              } else {
                this.invalidate(template`${prop} must be a ${type} uri`, undefined, '116');
              }
            }
          });
        }
      ...
      ARYS.forEach((prop) => {
        if (this[prop] !== undefined) {
          if (!Array.isArray(this[prop])) {
            this.invalidate(template`${prop} must be an array`, undefined, '117');
          }
      ...
      BOOL.forEach((prop) => {
        if (this[prop] !== undefined) {
          if (typeof this[prop] !== 'boolean') {
            this.invalidate(template`${prop} must be a boolean`, undefined, '118');
          }
        }
      });
    }
    ...
    whens() {
      Object.entries(WHEN).forEach(([when, [property, value]]) => {
        if (this[when] !== undefined && this[property] === undefined) {
          this.invalidate(template`${property} is mandatory property when ${when} is provided`, undefined, '119');
        }
      ...
      Object.entries(ENUM).forEach(([prop, fn]) => {
        ...
        if (this[prop] !== undefined) {
          ...
          if (isAry && !this[prop].every((val) => only[method](val))) {
            if (length) {
              this.invalidate(template`${prop} can only contain ${[[...only], { type: 'disjunction' }]}`, undefined, '120');
            } else {
              this.invalidate(template`${prop} must be empty (no values are allowed)`, undefined, '121');
            }
          } else if (!isAry && !only[method](this[prop])) {
            if (length) {
              this.invalidate(template`${prop} must be ${[[...only], { type: 'disjunction' }]}`, undefined, '122');
            } else {
              this.invalidate(template`${prop} must not be provided (no values are allowed)`, undefined, '123');
            }
          }
        }
      });
    ...
    webMessageUris() {
      ...
      this.web_message_uris.forEach((uri) => {
        ...
        try {
          ...
          this.invalidate('web_message_uris must only contain valid uris', undefined, '124');
        }
        if (!['https:', 'http:'].includes(protocol)) {
          this.invalidate('web_message_uris must only contain web uris', undefined, '125');
        }
        if (origin !== uri) {
          this.invalidate('web_message_uris must only contain origins', undefined, '126');
        }
      });
    }
    ...
    redirectUris(uris = this.redirect_uris, label = 'redirect_uris') {
      uris.forEach((redirectUri) => {
        ...
        try {
          ...
          this.invalidate(template`${label} must only contain valid uris`, undefined, '127');
        }

        const { hash } = url.parse(redirectUri);

        if (hash) {
          this.invalidate(template`${label} must not contain fragments`, undefined, '128');
        }

        switch (this.application_type) { // eslint-disable-line default-case
          case 'web': {
            if (!['https:', 'http:'].includes(protocol)) {
              this.invalidate(template`${label} must only contain web uris`, undefined, '129');
            }

            if (this.grant_types.includes('implicit')) {
              if (protocol === 'http:') {
                this.invalidate(template`${label} for web clients using implicit flow MUST only register URLs using the https scheme`, 'implicit-force-https', '130');
              }

              if (hostname === 'localhost') {
                this.invalidate(template`${label} for web clients using implicit flow must not be using localhost`, 'implicit-forbid-localhost', '131');
              }
            }
            break;
          }
            ...
            switch (protocol) {
              case 'http:': // Loopback Interface Redirection
                if (!LOOPBACKS.has(hostname)) {
                  this.invalidate(template`${label} for native clients using http as a protocol can only use loopback addresses as hostnames`, undefined, '132');
                }
                break;
              case 'https:': // Claimed HTTPS URI Redirection
                if (LOOPBACKS.has(hostname)) {
                  this.invalidate(template`${label} for native clients using claimed HTTPS URIs must not be using ${hostname} as hostname`, undefined, '133');
                }
                break;
              default: // Private-use URI Scheme Redirection
                if (!protocol.includes('.')) {
                  this.invalidate(template`${label} for native clients using Custom URI scheme should use reverse domain name based scheme`, undefined, '134');
                }
            }
            break;
          }
      ...
      if (this.contacts) {
        this.contacts.forEach((contact) => {
          if (!W3CEmailRegExp.test(contact)) {
            this.invalidate('contacts can only contain email addresses', undefined, '135');
          }
        });
      }
    }
    ...
    scopes() {
      if (this.scope) {
        ...
        parsed.forEach((scope) => {
          if (!scopes.has(scope)) {
            this.invalidate('scope must only contain Authorization Server supported scope values', undefined, '136');
          }
        });
    ...
    syntax() {
      for (const [prop, regexp] of Object.entries(SYNTAX)) {
        if (regexp.exec(this[prop])) {
          this.invalidate(template`invalid ${prop} value`, undefined, '137');
        }
      }
    }
  }
