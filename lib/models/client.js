...
const validateJWKS = (jwks) => {
  if (jwks !== undefined) {
    if (!Array.isArray(jwks?.keys) || !jwks.keys.every(isPlainObject)) {
      throw new InvalidClientMetadata(undefined, '002');
    }
  }
};
...
export default function getClient(provider) {
  ...
  async function sectorValidate(client) {
    ...
    const { statusCode, body } = await request.call(provider, {
      ...
      throw new InvalidClientMetadata(err.message, '003');
    });

    if (statusCode !== 200) {
      throw new InvalidClientMetadata(undefined, '004', [statusCode, STATUS_CODES[statusCode]]);
    }

    try {
      if (!Array.isArray(body)) throw new InvalidClientMetadata(undefined, '005');
      if (client.responseTypes.length) {
        const match = client.redirectUris.every((uri) => body.includes(uri));
        if (!match) throw new InvalidClientMetadata(undefined, '006');
      }
      ...
      if (
        ...
        if (!body.includes(client.jwksUri)) throw new InvalidClientMetadata(undefined, '007');
      }
    } catch (err) {
    ...
    async refresh() {
      ...
      if (!this.lock) {
        this.lock = (async () => {
          ...
          if (statusCode !== 200) {
            throw new InvalidClientMetadata(undefined, '008', [statusCode, STATUS_CODES[statusCode]]);
          }
          ...
          delete this.lock;
          const errorDescription = 'client JSON Web Key Set failed to be refreshed';
          if (err instanceof InvalidClientMetadata) {
            /* eslint-disable no-param-reassign */
            err.error_detail = err.error_description;
            err.error_description = errorDescription;
            /* eslint-enable no-param-reassign */
            throw err;
          }
          const e = new InvalidClientMetadata(err.error_description || err.message, '009');
          e.error_description = errorDescription;
          throw e;
        });
      }
  ...
  function addStatic(metadata) {
    if (!isPlainObject(metadata) || !metadata.client_id) {
      throw new InvalidClientMetadata(undefined, '010');
    }

    if (staticCache.has(metadata.client_id)) {
      throw new InvalidClientMetadata(undefined, '011');
    }
  ...
  class Client {
    ...
    checkClientSecretExpiration(message, errorOverride, errno, variables) {
      ...
      if (epochTime() - clockTolerance >= this.clientSecretExpiresAt) {
        const err = new InvalidClient(message, `client_id ${this.clientId} client_secret expired at ${this.clientSecretExpiresAt}`, errno, variables);
        if (errorOverride) {
