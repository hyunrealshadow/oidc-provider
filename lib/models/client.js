...
const validateJWKS = (jwks) => {
  if (jwks !== undefined) {
    if (!Array.isArray(jwks?.keys) || !jwks.keys.every(isPlainObject)) {
      throw new InvalidClientMetadata('client JSON Web Key Set is invalid', undefined, '002');
    }
  }
};
...
export default function getClient(provider) {
  ...
  async function sectorValidate(client) {
    ...
    const { statusCode, body } = await request.call(provider, {
      ...
      throw new InvalidClientMetadata('could not load sector_identifier_uri response', err.message, '003');
    });

    if (statusCode !== 200) {
      throw new InvalidClientMetadata(template`unexpected sector_identifier_uri response status code, expected 200 OK, got ${statusCode} ${STATUS_CODES[statusCode]}`, undefined, '004');
    }

    try {
      if (!Array.isArray(body)) throw new InvalidClientMetadata('sector_identifier_uri must return single JSON array', undefined, '005');
      if (client.responseTypes.length) {
        const match = client.redirectUris.every((uri) => body.includes(uri));
        if (!match) throw new InvalidClientMetadata('all registered redirect_uris must be included in the sector_identifier_uri response', undefined, '006');
      }
      ...
      if (
        ...
        if (!body.includes(client.jwksUri)) throw new InvalidClientMetadata("client's jwks_uri must be included in the sector_identifier_uri response", undefined, '007');
      }
    } catch (err) {
      if (err instanceof InvalidClientMetadata) {
        throw err;
      }
      throw new InvalidClientMetadata(err.message, undefined, '099');
    }
  }
    ...
    async refresh() {
      ...
      if (!this.lock) {
        this.lock = (async () => {
          ...
          if (statusCode !== 200) {
            throw new InvalidClientMetadata('', template`unexpected jwks_uri response status code, expected 200 OK, got ${statusCode} ${STATUS_CODES[statusCode]}`, '008');
          }
          ...
          if (err instanceof InvalidClientMetadata) {
            // eslint-disable-next-line no-param-reassign
            err.error_description = 'client JSON Web Key Set failed to be refreshed';
            throw err;
          }
          throw new InvalidClientMetadata('client JSON Web Key Set failed to be refreshed', err.error_description || err.message, '009');
        });
      }
  ...
  function addStatic(metadata) {
    if (!isPlainObject(metadata) || !metadata.client_id) {
      throw new InvalidClientMetadata('client_id is mandatory property for statically configured clients', undefined, '010');
    }

    if (staticCache.has(metadata.client_id)) {
      throw new InvalidClientMetadata('client_id must be unique amongst statically configured clients', undefined, '011');
    }
  ...
  class Client {
    ...
    checkClientSecretExpiration(message, errorOverride) {
      ...
      if (epochTime() - clockTolerance >= this.clientSecretExpiresAt) {
        const err = new InvalidClient(message, template`client_id ${this.clientId} client_secret expired at ${this.clientSecretExpiresAt}`, '099');
        if (errorOverride) {
