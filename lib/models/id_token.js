  ...
  return class IdToken {
    ...
    async issue({ use, expiresAt = null } = {}) {
      ...
      const signed = await (async () => {
        ...
        if (alg.startsWith('HS')) {
          if (use !== 'authorization') { // handled in checkResponseMode
            client.checkClientSecretExpiration(true, undefined, messages.sig[use], [alg]);
          }
      ...
      if (/^(A|dir$)/.test(encryption.alg)) {
        if (use !== 'authorization') { // handled in checkResponseMode
          client
            .checkClientSecretExpiration(true, undefined, messages.enc[use], [encryption.alg]);
        }
      }
      ...
      if (!encryptionKey) {
        throw new InvalidClientMetadata(undefined, '012', [encryption.alg]);
      }
    ...
    static async validate(jwt, client) {
      ...
      let keyOrStore;
      if (alg.startsWith('HS')) {
        client.checkClientSecretExpiration(true, undefined, '004');
