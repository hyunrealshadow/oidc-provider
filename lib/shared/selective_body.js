...
is not recommended, resolving to use req.body or request.body instead');
      ...
      throw new InvalidRequest('failed to parse the request body', undefined, undefined, '067');
    }

    await next();
  } else if (ctx.get('content-type')) {
    throw new InvalidRequest(template`only ${cty} content-type bodies are supported on ${ctx.method} ${ctx.path}`, undefined, undefined, '068');
  } else {
    ctx.oidc.body = {};
    await next();
  }
