  ...
  return {
    ...
    middleware: [
      ...
      async function findClientId(ctx, next) {
        ...
        if (ctx.headers.authorization !== undefined) {
          ...
          if (parts.length !== 2 || parts[0].toLowerCase() !== 'basic') {
            throw new InvalidRequest('invalid authorization header value format', undefined, undefined, '070');
          }
          ...
          if (i === -1) {
            throw new InvalidRequest('invalid authorization header value format', undefined, undefined, '071');
          }
          ...
          try {
            ...
            throw new InvalidRequest('client_id and client_secret in the authorization header are not properly encoded', undefined, undefined, '072');
          }

          if (clientId !== undefined && ctx.oidc.authorization.clientId !== clientId) {
            throw new InvalidRequest('mismatch in body and authorization client ids', undefined, undefined, '073');
          }

          if (!ctx.oidc.authorization.clientSecret) {
            throw new InvalidRequest('client_secret must be provided in the Authorization header', undefined, undefined, '074');
          }

          if (clientSecret !== undefined) {
            throw new InvalidRequest('client authentication must only be provided using one mechanism', undefined, undefined, '075');
          }
        ...
        if (clientAssertion !== undefined) {
          if (clientSecret !== undefined || ctx.headers.authorization !== undefined) {
            throw new InvalidRequest('client authentication must only be provided using one mechanism', undefined, undefined, '076');
          }
          ...
          try {
            ...
            throw new InvalidRequest('invalid client_assertion format', undefined, undefined, '077');
          }

          if (!sub) {
            throw new InvalidClientAuth('sub (JWT subject) must be provided in the client_assertion JWT', '001');
          }

          if (clientId && sub !== clientId) {
            throw new InvalidRequest('subject of client_assertion must be the same as client_id provided in the body', undefined, undefined, '078');
          }

          if (clientAssertionType === undefined) {
            throw new InvalidRequest('client_assertion_type must be provided', undefined, undefined, '079');
          }

          if (clientAssertionType !== assertionType) {
            throw new InvalidRequest(template`client_assertion_type must have value ${assertionType}`, undefined, undefined, '080');
          }
        ...
        if (!ctx.oidc.authorization.clientId) {
          throw new InvalidRequest('no client authentication mechanism provided', undefined, undefined, '081');
        }
      ...
      async function loadClient(ctx, next) {
        ...
        if (!client) {
          throw new InvalidClientAuth('client not found', '002');
        }
      ...
      async function auth(ctx, next) {
        ...
        if (!methods.includes(clientAuthMethod)) {
          throw new InvalidClientAuth('the provided authentication mechanism does not match the registered client authentication method', '003');
        }
        ...
        switch (clientAuthMethod) { // eslint-disable-line default-case
          ...
          case 'client_secret_post': {
            ...
            if (!matches) {
              throw new InvalidClientAuth('invalid secret provided', '004');
            }
          ...
          case 'tls_client_auth': {
            ...
            if (!cert) {
              throw new InvalidClientAuth('client certificate was not provided', '005');
            }

            if (!certificateAuthorized(ctx)) {
              throw new InvalidClientAuth('client certificate was not verified', '006');
            }
            ...
            for (const [prop, key] of Object.entries({
              ...
              if (value) {
                if (!certificateSubjectMatches(ctx, key, value)) {
                  throw new InvalidClientAuth('certificate subject value does not match the registered one', '007');
                }
                break;
              }
            }
          ...
          case 'self_signed_tls_client_auth': {
            ...
            if (!cert) {
              throw new InvalidClientAuth('client certificate was not provided', '008');
            }
            ...
            if (!match) {
              throw new InvalidClientAuth('unregistered client certificate provided', '009');
            }
