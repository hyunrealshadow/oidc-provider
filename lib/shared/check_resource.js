...
export default async function checkResource(ctx, next) {
  ...
  for (const identifier of resource) {
    ...
    try {
      ...
      throw new InvalidTarget('resource indicator must be an absolute URI', undefined, '006');
    }
    ...
    if (href.includes('#')) {
      throw new InvalidTarget('resource indicator must not contain a fragment component', undefined, '007');
    }
