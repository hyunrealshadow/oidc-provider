...
describe('template helper', () => {
  ...
  it('parse template string to get template information with intl format array', () => {
    ...
    expect(result.variables).to.eql({ key0: [['A1', 'A2', 'A3'], { type: 'conjunction' }], key1: [['B1', 'B2', 'B3'], { type: 'disjunction' }] });
  ...
  it('parse template string to get template information with intl format array and pluralize', () => {
    ...
    expect(result.template).to.eq('Test {{key0, intlPluralize(one: \'param\', many: \'params\', length: 1)}} Test {{key1, intlPluralize(one: \'param\', many: \'params\', length: 2)}}');
    expect(result.variables).to.eql({ key0: [['param', 'params', 1], { type: 'pluralize' }], key1: [['param', 'params', 2], { type: 'pluralize' }] });
